...
void file_header(darray *da, std::size_t text_size) {
    ...
    push_dvalue(da, uint64_t, START_ADDR + 0x40 + 0x38); // execution starts here, section text
...
void add_section_names(darray *da, int *section_offsets) {
	...
	for (int i = 0; i < section_names.size(); i++) {
		push_string(da, section_names[i].size() + 1, section_names[i].c_str()); // include null terminator
	}
}

void section_header_entry(darray *da, Elf64_Word sh_name, Elf64_Word sh_type,
                          Elf64_Xword sh_flags, Elf64_Addr sh_addr, Elf64_Off sh_offset,
                          Elf64_Xword sh_size, Elf64_Word sh_link, Elf64_Word sh_info,
...
void section_header_table(darray *da, std::size_t text_size, int *section_offsets) {
  ...
  section_header_entry(da, section_offsets[1], SHT_PROGBITS, (SHF_ALLOC | SHF_EXECINSTR),
	...
	//names section
  section_header_entry(da, section_offsets[0], SHT_STRTAB, 0,
                       0, 0x40 + 0x38 + text_size, section_name_size,
                       0, 0, 0, 0);
}
...
void write_elf(darray *text, std::size_t text_size) {
  ...
  DEBUG_ELF_SECTION(section_header_table(da, text_size,
                                         section_offsets), da, "Section header table");
